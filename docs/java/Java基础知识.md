...
## 1. 面向对象(object-oriented)和面向过程(procedure-oriented procedural programming)的区别
...
* Low Coupling: module should be as independent as possible
...
## 2. Java 语言有哪些特点? Feature

1. 简单易学；
2. 面向对象（封装，继承，多态）；
3. 平台无关性（ Java 虚拟机实现平台无关性）；-> JAVA virtual machine
4. 可靠性；
5. 安全性；(static type check -> prevent some type error)
...
Java is semi-complied language -> compiled it into a class file (bytecode file) and then the JVM excute the bytecode interpretively.
...
（JVM classloader will load the bytecode file and interpretively executee the code via interpretor, turn them into binary machine code）

而且，有些方法和代码块是经常需要被调用的(也就是所谓的热点代码)，所以后面引进了 JIT 编译器，而 JIT 属于运行时编译。当 JIT 编译器完成第一次编译后，其会将字节码对应的机器码保存下来，下次可以直接使用。而我们知道，机器码的运行效率肯定是高于 Java 解释器的。这也解释了我们为什么经常会说 Java 是编译与解释共存的语言。

JIT -> hotspot code (binary machine code saved)
...
* JDK includes JRE, javac and other tools (javadoc)
...
- 都是面向对象的语言，都支持封装、继承和多态 (encapsulation, inheritance, polymorhism)

- Java 不提供指针来直接访问内存，程序内存更加安全 (references vs pointers)
...
> single inheritance (abstract class) -> implements several interface 
>
> Multiple inheritance (extends several class)



- Java 有自动内存管理机制，不需要程序员手动释放无用内存

  > JAVA has GC responsible for the ato memory management, no need to del/ destruct objects to release useless memory
...
> (entry point) -> 对于javaSE 开发的应用程序来讲 （例如 飞机大战）main() 函数就是我们的entry point （有main 函数方法的类）

而在 Java 小程序中，这个主类是一个继承自系统类 JApplet 或 Applet 的子类。应用程序的主类不一定要求是 public 类，但小程序的主类要求必须是 public 类。主类是 Java 程序执行的入口点。
...
## 12. String StringBuffer 和 StringBuilder 的区别是什么?(用来解释abstractclass) String 为什么是不可变的?
...
>
> ![image-20201202172354866](Java基础知识.assets/image-20201202172354866.png)
>
> > ```
> > Objects.equals(new String(""),new String("")); // 首先判断 是否有一个为空 或者同时为空 然后再判断 内容是否一致
> > ```
> >
> > equal-to operator ( *==* )  is to compare if these two reference point to the same object / the same memory address (for reference type speaking)
...
**there might be collision in the hashSet.**





## 28. 为什么 Java 中只有值传递 (method)？ passed by reference or by value
...
![example 1 ](Java基础知识.assets/1-20201202174409393-6949677.)
...
![example 2](Java基础知识.assets/1-6949677.)
...
![img](Java基础知识.assets/1-20201202175437994)

交换之后：

![img](Java基础知识.assets/1-20201202174409602-6949677.)

通过上面两张图可以很清晰的看出： **方法并没有改变存储在变量 s1 和 s2 中的对象引用。swap 方法的参数 x 和 y 被初始化为两个对象引用的拷贝，这个方法交换的是这两个拷贝**。 could not change the value of reference
...
- 一个方法不能修改一个基本数据类型的参数（即数值型或布尔型）。
  ...
  > A method could not modify the value of a basic data type (passed into)

  

- 一个方法可以改变一个对象参数的状态。

  > A method could change the state of an object parameters it passed into 

  

- 一个方法不能让对象参数引用一个新的对象。

> A method could not what the reference it points to
...
Serialize: Java Object -> String (it could be transmitted in byteStream)

DeSerialize: String (ByteStream) -> JAVA objects
...
## 38. 深拷贝 (new Object) vs 浅拷贝 (pass by reference)
